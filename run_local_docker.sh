...
# Script to build and run the Test mcp server Traia MCP Server locally in Docker
...
IMAGE_NAME="test-mcp-server-traia-mcp-server"
CONTAINER_NAME="test-mcp-server-traia-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Test mcp server Traia MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Test mcp server Traia MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Test mcp server Traia MCP Server is running!${NC}"
echo
