#!/usr/bin/env python3
"""
Test mcp server Traia MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('test-mcp-server-traia_mcp')
...
logger.info("="*80)
logger.info(f"Test mcp server Traia MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://date.nager.at")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Test mcp server Traia MCP Server", host="0.0.0.0")
...
# - TEST_MCP_SERVER_TRAIA_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="test-mcp-server-traia-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "test-mcp-server-traia-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Test mcp server Traia MCP Server")
    logger.info("="*80)
